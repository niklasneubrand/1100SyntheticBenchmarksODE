...
save_name="localOptimizationV2_3"

# configs
configs="'atol', 1e-6, 'rtol', 1e-6, 'maxsteps', 1e6, 'add_c', 80, 'MaxIter', 1e5, 'TolFun', 1e-6"
...
nohup matlab-R2021a -r "localOptimizationV2Multi('$fast2Dir', 'normal', '$save_name', $configs); exit;" > $log_file_fast2 2>&1 &
sleep 5  # add a small delay to avoid overwhelming the system
echo "Processing directory: $slow2Dir"
nohup matlab-R2021a -r "localOptimizationV2Multi('$slow2Dir', 'normal', '$save_name', $configs); exit;" > $log_file_slow2 2>&1 &