...
save_name="localOptimization_restricted_V2"

# configs
variant="restricted"
configs="'atol', 1e-8, 'rtol', 1e-8, 'eq_tol', 1e-8"
...
for template_dir in "$base_dir"/*/; do
    if [ -d "$template_dir" ]; then
        ...
        nohup matlab-R2021a -r "localOptimizationVariantsMulti('$variant', '$template_dir', 'Final', '$save_name', $configs); exit;" > $log_file 2>&1 &
...
nohup matlab-R2021a -r "localOptimizationVariantsMulti('$variant', '$fast2Dir', 'normal', '$save_name', $configs); exit;" > $log_file_fast2 2>&1 &
sleep 5  # add a small delay to avoid overwhelming the system
echo "Processing directory: $slow2Dir"
nohup matlab-R2021a -r "localOptimizationVariantsMulti('$variant', '$slow2Dir', 'normal', '$save_name', $configs); exit;" > $log_file_slow2 2>&1 &