...
restarts=("Bachmann_MSB2011" "Merkle_PCB2016")
...
for template_dir in "$base_dir"/*/; do
    if [ -d "$template_dir" ]; then
        ...
        # Check if the folder name is in the restarts array
        if [[ ! " ${restarts[@]} " =~ " ${folder_name} " ]]; then
            continue
        fi
...
# nohup matlab-R2021a -r "localOptimizationMulti('$fast2Dir', 'normal'); exit;" > $log_file_fast2 2>&1 &
# nohup matlab-R2021a -r "localOptimizationMulti('$slow2Dir', 'normal'); exit;" > $log_file_slow2 2>&1 &