...
failed_simulations=(49 37 31 33 41 17 7 2 10 48 40)
...
for sim in "${failed_simulations[@]}"; do
    ...
    failed_sim_dir="$failed_dir/${model_prefix}_RS${sim_formatted}_failed"
    ...
    nohup matlab-R2021a -r "initRealisticBenchmarks; cd('$target_dir'); arManyRealisticDesigns(${sim}, 'rngSeed', 2000010, 'calculateRSTemplate', false); exit();" </dev/null >/dev/null 2>&1 &
done
